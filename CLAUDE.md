...
> 세션이 종료되어도 문서를 통해 컨텍스트를 유지한다.

## 세션 시작 시 필수 확인
1. `/docs/progress.md` - 현재 상태, 진행 중 작업
2. `/docs/spec/mvp-features.md` - 완료/미완료 체크리스트

## 기능 구현 순서
1. Cucumber 시나리오 작성 → StepDefs 작성
2. 도메인/UseCase 구현 → Repository/Controller 구현
3. 테스트 실행 (`./gradlew test --tests "com.hoops.acceptance.*"`)
4. 커밋 → 문서 업데이트

## 작업 완료 시 필수 업데이트
- `/docs/progress.md` - 최근 완료에 추가
- `/docs/spec/mvp-features.md` - 체크리스트 표시

## 원칙
- 문서가 진실의 원천이다
- 불확실하면 문서를 먼저 확인한다
- 작업 완료 후 반드시 문서를 업데이트한다