...
### 2.4 경기 복구 (취소 복구)
- [x] 경기 복구 API (`POST /api/matches/{matchId}/reactivate`)
- [x] 복구 시간 제한 (취소 후 1시간 이내)
- [x] 경기 날짜 검증 (지난 경기 복구 불가)
- [x] 경기 복구 Cucumber 테스트 (match-reactivate.feature)
...
| 날짜 | 변경 내용 | 작성자 |
|------|----------|--------|
| 2026-01-13 | 경기 복구(Reactivate) 기능 추가, 진행률 89% | Claude |
