...
3. 검색된 경기들을 지도에 마커로 표시
   - 각 마커는 코트 위치에 표시
   - 같은 코트에 여러 경기가 있으면 마커 클러스터링 또는 숫자 표시
...
| **마커 숫자 표시** | 같은 위치에 여러 경기가 있으면 숫자 배지로 표시 (예: "3") |
| **마커 클러스터링** | 가까운 위치의 마커들은 숫자로 그룹화 |
...
□ 마커 클러스터링 구현
□ 같은 위치 여러 경기 처리 로직 (마커 숫자 표시, 경기 목록 팝업)
□ 실시간 위치 업데이트 (선택적)
□ 알림 발송 시스템 구현
...
□ 지도 로딩 속도 최적화
□ 마커 클릭 반응 속도
□ 같은 위치에 여러 경기가 있을 때 직관적인 경기 선택 UI
□ 시간대별 경기 정렬 (가까운 시간 순)
□ 신청 버튼 클릭 후 피드백 명확
□ 승인 대기 상태 명확히 표시
